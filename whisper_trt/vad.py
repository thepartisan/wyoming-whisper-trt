...
def load_vad(model_path: Optional[str] = None, download: bool = True, force_cpu: bool = False) -> SileroVAD:
    """
    ...
    if not model_path.exists():
        ...
        logger.info(f"Model not found at {model_path}. Downloading from {SILERO_VAD_ONNX_URL}")
    ...
    if not check_file_md5(str(model_path), SILERO_VAD_ONNX_MD5_CHECKSUM):
        error_msg = f"The MD5 checksum for {model_path} does not match the expected value."
        logger.error(error_msg)
        raise RuntimeError(error_msg)
